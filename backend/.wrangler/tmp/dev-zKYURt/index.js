...
// .wrangler/tmp/bundle-mCsOw4/checked-fetch.js
...
// .wrangler/tmp/bundle-mCsOw4/strip-cf-connecting-ip-header.js
...
var fetchRepository = /* @__PURE__ */ __name(async (repoUrl, branch, env) => {
...
// src/checkers/cloudflare.ts
var checkCloudflareReadiness = /* @__PURE__ */ __name((repoFiles) => {
  const issues = [];
  ...
  const hasSQLite = hasPackageJson && (hasPackageJson.content.includes('"sqlite3"') || hasPackageJson.content.includes('"sqlite"'));
  const hasLocalStorage = repoFiles.some(
    (f) => f.content.includes("fs.") || f.content.includes("multer")
  );
  ...
  const hasPortBinding = repoFiles.some(
    (f) => f.content.includes("app.listen(") || f.content.includes(".listen(")
  );
  if (hasPortBinding) {
    issues.push({
      category: "runtime",
...
// src/checkers/azure.ts
var checkAzureReadiness = /* @__PURE__ */ __name((repoFiles) => {
  const issues = [];
  const hasAzureConfig = repoFiles.some((f) => f.path === "azure.yaml" || f.path === ".azure/config");
  const hasPackageJson = repoFiles.find((f) => f.path === "package.json");
  const hasSQLite = hasPackageJson && (hasPackageJson.content.includes('"sqlite3"') || hasPackageJson.content.includes('"sqlite"'));
  const hasLocalStorage = repoFiles.some(
    (f) => f.content.includes("fs.writeFile") || f.content.includes("multer")
  );
...
var analyzeRepository = /* @__PURE__ */ __name(async (repoUrl, targetPlatform, branch, env) => {
...
var getChecker = /* @__PURE__ */ __name((platform) => {
  switch (platform) {
    case "cloudflare":
...
var storeAnalysis = /* @__PURE__ */ __name(async (result, repoFiles, env) => {
...
// src/porters/cloudflare.ts
var portToCloudflare = /* @__PURE__ */ __name((repoFiles, issues) => {
  const portedFiles = [];
...
var createWranglerConfig = /* @__PURE__ */ __name(() => {
...
var convertExpressToHono = /* @__PURE__ */ __name((repoFiles) => {
...
}, "convertExpressToHono");
var convertSQLiteToD1 = /* @__PURE__ */ __name((repoFiles) => {
...
var convertLocalStorageToR2 = /* @__PURE__ */ __name((repoFiles) => {
...
}, "convertLocalStorageToR2");
var updatePackageJson = /* @__PURE__ */ __name((repoFiles) => {
...
// src/porters/azure.ts
var portToAzure = /* @__PURE__ */ __name((repoFiles, issues) => {
  const portedFiles = [];
...
var createAzureConfig = /* @__PURE__ */ __name(() => {
...
var createAzureSQLConfig = /* @__PURE__ */ __name(() => {
...
var createBlobStorageConfig = /* @__PURE__ */ __name(() => {
...
}, "createBlobStorageConfig");
var updatePackageJsonForAzure = /* @__PURE__ */ __name((repoFiles) => {
  const packageFile = repoFiles.find((f) => f.path === "package.json");
  if (!packageFile) {
    return {
      path: "package.json",
...
var createDeploymentDocs = /* @__PURE__ */ __name(() => {
...
var generatePatch = /* @__PURE__ */ __name(async (analysisId, env) => {
  ...
  const repoFiles = JSON.parse(await filesData.text());
  const issues = JSON.parse(analysis.issues);
  const porter = getPorter(analysis.target_platform);
...
var getPorter = /* @__PURE__ */ __name((platform) => {
  switch (platform) {
    case "cloudflare":
...
var generateGitPatch = /* @__PURE__ */ __name((result) => {
  let patch = "";
...
// .wrangler/tmp/bundle-mCsOw4/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-mCsOw4/middleware-loader.entry.ts
